...
class THDInputKwargs(TypedDict):
    """Keyword arguments for packed sequence inputs to transformer-engine layers as returned by the DataCollatorWithFlattening class.
    ...
    def forward(
        ...
        **kwargs: Unpack[THDInputKwargs],
        ...
        Args:
            ...
            **kwargs: Unpack[THDInputKwargs]: The THD input kwargs.
        """
        all_hidden_states = () if output_hidden_states else None
        # Overwrites rope embeddings with the THD input kwargs.
        te_rope_emb = self.rotary_embeddings(max_seq_len=kwargs["cu_seq_lens_q"][-1]).cuda()

        for layer_module in self.layers:
            if output_hidden_states:
                all_hidden_states = (*all_hidden_states, hidden_states)

            # Here layer_module is a TransformerLayer. Note there is not attention mask here because we packed everything into a single tensor.
            hidden_states = layer_module(
                hidden_states=hidden_states.squeeze(0),
                rotary_pos_emb=te_rope_emb,
                cu_seqlens_q=kwargs["cu_seq_lens_q"],
                cu_seqlens_kv=kwargs["cu_seq_lens_k"],
                max_seqlen_q=kwargs["max_length_q"],
                max_seqlen_kv=kwargs["max_length_k"],
            )
