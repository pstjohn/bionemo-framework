...
def preload_csv_files(  # noqa: C901, D103
    data_path, tokenizer, save_path, chunk_size, num_workers=cpu_count(), min_cds_len=100, max_cds_len=1.5e5
):
    ...
    for file_name in sorted(os.listdir(data_path)):
        if file_name.endswith(".csv"):
            ...
            if os.path.exists(cached_file_path):
                ...
                with Pool(cpu_count()) as pool:
                    ...
                    for bi in range(0, df.shape[0], bs):
                        ...
                        tokenized_sequences += list(
                            tqdm(
                                pool.imap(tokenize_sequence, [(seq, tokenizer) for seq in cds_sequences]),
                ...
                df = df.with_columns(pl.col("cds_tokens").str.len_chars().alias("cds_tokens_length"))
                df.write_parquet(cached_file_path)
                print("Saved")
            df = df.with_columns(
                pl.col("cds_tokens")
                .map_elements(lambda x: np.frombuffer(x.encode("utf-8"), dtype=np.uint8), return_dtype=pl.Object)
                .alias("cds_tokens_encoded")
            )
            ...
            if num_workers == 0:
                for chunk_id, (file_path, start_row_idx, end_row_idx) in tqdm(
                    remaining_chunks_metadata, total=len(remaining_chunks_metadata)
                ):
...
def save_chunk(chunk_id, chunk_info, save_path, token_lengths, taxid_series, sequences_flat):  # noqa: D103
