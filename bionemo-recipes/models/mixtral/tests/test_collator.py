...
"""Tests for DataCollatorWithFlattening and TokenPackingDataset for causal LM (Mixtral)."""
...
def tokenizer():
    """Load the Mixtral tokenizer for testing."""
    return AutoTokenizer.from_pretrained("NeuralNovel/Mini-Mixtral-v0.2")
...
def test_data_collator_with_flattening_causal_lm(tokenizer):
    ...
    features = [
        {"input_ids": [1, 100, 200, 300, 2]},  # 5 tokens
        {"input_ids": [1, 400, 500, 600, 700, 2]},  # 6 tokens
        {"input_ids": [1, 800, 900, 2]},  # 4 tokens
    ]
    ...
    # The separator is placed at cu_seq_lens_q[1:-1], i.e., positions 5 and 11
...
def test_data_collator_pad_to_multiple_of(tokenizer):
    ...
    features = [
        {"input_ids": [1, 100, 200, 300, 2]},  # 5 tokens
        {"input_ids": [1, 400, 500, 2]},  # 4 tokens
    ]
...
def test_split_sample_by_num_tokens_basic():
    ...
    sample = {"input_ids": [1, 100, 200, 300, 400, 500, 2]}
    first, remaining = _split_sample_by_num_tokens(sample, 3)

    assert first["input_ids"] == [1, 100, 200]
    assert remaining["input_ids"] == [300, 400, 500, 2]


def test_split_sample_by_num_tokens_with_labels():
    """Test _split_sample_by_num_tokens preserves labels."""
    sample = {"input_ids": [1, 100, 200, 300, 2], "labels": [1, 100, 200, 300, 2]}
    first, remaining = _split_sample_by_num_tokens(sample, 2)

    assert first["input_ids"] == [1, 100]
    assert first["labels"] == [1, 100]
    assert remaining["input_ids"] == [200, 300, 2]
    assert remaining["labels"] == [200, 300, 2]


def test_split_sample_by_num_tokens_errors():
    """Test _split_sample_by_num_tokens raises errors for invalid inputs."""
    sample = {"input_ids": [1, 100, 200, 2]}
