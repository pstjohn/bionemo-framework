...
def test_train_dataloader_creation(tokenized_dataset):
    datamodule = MockDataModule(train_dataset=tokenized_dataset)
...
def test_padding_and_truncation(example_sequences):
    ...
    datamodule = MockDataModule(
        ...
        min_seq_length=4,
        max_seq_length=6,
        batch_size=3,  # Process all sequences in one batch to observe padding
    )
...
def test_validation_dataloader(tokenized_dataset):
    datamodule = MockDataModule(valid_dataset=tokenized_dataset)
    val_loader = datamodule.val_dataloader()
    assert isinstance(val_loader, DataLoader)
    assert val_loader.batch_size == datamodule.batch_size

    # Verify we can actually get a batch without errors
    batch = next(iter(val_loader))
    assert isinstance(batch["text"], torch.Tensor)


def test_test_dataloader(tokenized_dataset):
    datamodule = MockDataModule(test_dataset=tokenized_dataset)
...
def test_batch_collation(example_sequences):
    ...
    datamodule = MockDataModule(
        train_dataset=dataset,
        batch_size=3,  # Process all sequences in one batch
    )
